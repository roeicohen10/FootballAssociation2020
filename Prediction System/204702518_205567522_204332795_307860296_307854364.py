...
path = "C:\\Uni\\Mini-Project\\Prediction\\"  # Insert path here
...
train_match_data = pd.read_sql("SELECT * FROM Match WHERE season<>'2015/2016';", conn)
test_match_data = pd.read_sql("SELECT * FROM Match WHERE season='2015/2016';", conn)
...
train_match_data.dropna(subset=rows, inplace=True)
test_match_data.dropna(subset=rows, inplace=True)


print("-----------------train fifa data fetching-----------------------")
fifa_data = get_fifa_data(train_match_data, player_stats_data, data_exists=False)
print("-----------------test fifa data fetching-----------------------")
test_fifa_data = get_fifa_data(test_match_data, player_stats_data, data_exists=False)
print("-----------------done fetching data-----------------------")

print("-----------------train data-----------------------")
bk_cols = ['B365', 'BW', 'IW', 'LB', 'PS', 'WH', 'SJ', 'VC', 'GB', 'BS']
bk_cols_selected = ['B365', 'BW']
feables = create_feables(train_match_data, fifa_data, bk_cols_selected, get_overall=True)
inputs = feables.drop('match_api_id', axis=1)
labels = inputs.loc[:, 'label']
features = inputs.drop('label', axis=1)

print("-----------------test data-----------------------")
test_feables = create_feables(test_match_data, test_fifa_data, bk_cols_selected, get_overall=True)
test_inputs = test_feables.drop('match_api_id', axis=1)
test_labels = test_inputs.loc[:, 'label']
test_features = test_inputs.drop('label', axis=1)
...
print(accuracy_score(test_labels, KNN_preds))
print("-------------------done building KNeighbors model-------------------")

print("--------------------starting building Logistic model----------------------")
LOG = linear_model.LogisticRegression(multi_class="ovr", solver="sag", class_weight='balanced')
LOG_model = LOG.fit(features, labels)
LOG_preds = LOG.predict(test_features)
...
print(accuracy_score(test_labels, RF_preds))
